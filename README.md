...
- Download the `brb_timer.py` script from this repo to your computer running OBS. That's the only file required.
...
- Launch OBS.
    ...
    - Select the `Frameworks/` folder _inside_ the python installation folder.
        - On Windows, that path may look like: ``
        - On a Mac, possibly: `/opt/homebrew/Cellar/python@3.12/3.12.13_4/Frameworks`
...
- The script requires minimal Twitch OAuth access to make API calls and to process chat messages in your channel.
    ...
    - In your browser, review the requests permissions, and click <kbd>Okay</kbd> if acceptable.
        - You can also review the requested "scopes" in the `OAUTH_SCOPES` constant defined in `brb-timer.py`.
    ...
    - Twitch will redirect your browser back to a landing page with an auth token attached.
...
- `!back` - Only accessible to the broadcaster and mods. Ends the `!brb`, hides the on-screen timer (after a delay), and sends a chat message stating the "winner".
